...
import com.lmj.pojo.Article;
import com.lmj.pojo.Comment;
import com.lmj.pojo.User;
import com.lmj.service.ArticleServiceImpl;
import com.lmj.service.CommentServiceImpl;
import com.lmj.service.LeaveMessageServiceImpl;
import com.lmj.service.UserServiceImpl;
import com.lmj.utils.MyUtils;
...
public class ArticleController {
    ...
    @PostMapping("/admin/articleSearch")
    public String searchArticles(@RequestParam("key") String keyword,  Model model) {

        System.out.println(keyword);

        List<Article> articlesByTitle = articleService.queryArticleByTitle(keyword); // 使用正确的参数名

        System.out.println(articlesByTitle);

        model.addAttribute("articles", articlesByTitle);
        return "searchindex";
    }
...
*/
    ...
    */
    ...
    普通用户删除文章
     */
    @GetMapping("/common/deleteArticle/{id}")
    public String commonDeleteArticle(@PathVariable int id){
        articleService.deleteArticle(id);
        return "redirect:/common/allArticle/1";
    }
    ...
    /*
       普通用户更新自己的文章
        */
    @GetMapping("/common/toUpdateArticle/{id}")
    public String commonToUpdateArticle(@PathVariable int id,Model model){
        //将需要更新的文章信息传递过去
        Article article = articleService.queryArticleById(id);
        model.addAttribute("article",article);
        return "commons/commonUpdateArticle";
    }
    ...
    @PostMapping("/common/updateArticle")
    public String commonUpdateArticle(int id,String title,String author,String content,String tags,String type,int likes){
        articleService.updateArticle(id,title,author,content,tags,type,likes,formatTime(new Date()));
        return "redirect:/common/allArticle/1";
    }
    ...
    /*
       后台普通用户检索文章及分页
        */
    @RequestMapping("/common/articleSearch/{page}")
    public String commonArticleSearch(@PathVariable int page,String key,Model model,HttpSession session){
        //记录第一次传进来的title 用静态变量保存
        if (key!=null) title = key;
        //查询所有相关的文章
//        List<Article> articles = articleService.queryArticleByTitleLimit("%"+title+"%", 0, 9999);
        List<Article> articles = articleService.commonQueryArticleByTitleLimit("%"+title+"%", 0, 9999,(Integer)session.getAttribute("id"));
        int articleSize = articles.size();
        model.addAttribute("articleSize",articleSize);
        //一页展示多少条
        int pageSize = 10;
        ...
        //查询当前页数的文章信息
        int start = (page-1)*pageSize;
        List<Article> targetArticle = articleService.commonQueryArticleByTitleLimit("%"+title+"%", start, pageSize,(Integer)session.getAttribute("id"));
        model.addAttribute("articles",targetArticle);
        //上一页下一页的页数传递过去
        int next = page+1;
        int prev = page-1;
        ...
        model.addAttribute("next",next);
        model.addAttribute("prev",prev);
        return "commons/commonArticleSearch";
    }

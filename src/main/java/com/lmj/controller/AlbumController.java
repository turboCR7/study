...
public class AlbumController {
    ...
    /*
       所有图片界面和分页(后台)
       */
    @GetMapping("/common/allAlbum/{page}")
    ...
    @RequestMapping("/common/albumSearch/{page}")
   ...
   打开添加相册信息界面
    */
    @GetMapping("/common/addAlbum")
    public String toUpdateUserInfo() {
        return "commons/commonAddAlbumInfo";
    }
    /*
    保存普通用户添加相册信息
     */
    @PostMapping("/common/addAlbumInfo")
    public String addAlbumInfo(@RequestParam("name") String name, String introduction, @RequestParam("img") MultipartFile img, HttpSession session) {
        ...
        return "redirect:/common/addAlbum";
    }

    /*
       普通用户删除相册
        */
    @GetMapping("/common/deleteAlbum/{id}")
    public String commonDeleteAlbum(@PathVariable int id){
        albumService.deleteAlbum(id);
        return "redirect:/common/allAlbum/1";
    }

    /*
   用户更新相册
    */
    @GetMapping("/common/toUpdateAlbum/{id}")
    public String toUpdateAlbum(@PathVariable int id,Model model){
        //将需要更新的文章信息传递过去
        Album album = albumService.queryAlbumById(id);
        model.addAttribute("album",album);
//        albumService.deleteAlbum(id);
        return "commons/updateCommonAddAlbumInfo";
    }


    @PostMapping("/common/UpdateAlbum")
